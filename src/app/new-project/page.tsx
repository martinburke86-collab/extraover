...
export default async function NewProjectPage() {
  ...
  async function create(formData: FormData) {
    ...
    const blankPrelims = [
      ...
      ['Welfare & Offices','16.013', 'IT Equipment - Computers and Printers','Weeks'],
      ['Running Costs',    '16.015', 'Stationery and Consumables',        'Weeks'],
      ['Bond & Insurance', '16.069', 'Temporary Works Design',            'nr'],
    ]
      ...
      <div className="bg-surface-container-lowest rounded-xl shadow-2xl w-full max-w-lg overflow-hidden">
        <div className="px-8 py-6" style={{ background: '#1e3a5f' }}>
          <div className="flex items-center gap-3 mb-1">
            <div className="w-8 h-8 rounded bg-white/20 flex items-center justify-center text-white font-black text-[10px]">EO</div>
            <span className="text-white font-black text-lg uppercase tracking-tight">ExtraOver</span>
          </div>
          <p className="text-white/60 text-sm mt-1">Create a new project</p>
        </div>
        <form action={create} className="px-8 py-6 space-y-4">
          {[
            ...
            <div key={name as string}>
              <label className="block text-xs font-bold text-on-surface-variant uppercase tracking-wide mb-1.5">{label as string}</label>
              <input name={name as string} placeholder={placeholder as string} required={!!required}
          ...
          <div className="pt-2 space-y-2">
            <button type="submit"
              className="w-full text-white rounded py-3 font-bold text-sm uppercase tracking-tight"
