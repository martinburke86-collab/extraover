...
export function PageHeader({ title, subtitle, actions }: {
  ...
  return (
    <header className="h-14 flex items-center justify-between px-8 bg-white border-b border-outline-variant/20 shrink-0">
      <div>
        <h2 className="text-lg font-black text-on-surface tracking-tight uppercase">{title}</h2>
        {subtitle && (
          <div className="flex items-center gap-1.5 text-[10px] text-on-surface-variant font-medium uppercase tracking-widest mt-0.5">
            {subtitle}
          </div>
        )}
      </div>
